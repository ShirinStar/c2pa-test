use c2pa::{
    assertions::{c2pa_action, Action, Actions, User},
...
fn write_assertion() -> Result<(), Error> {
    ...
    let actions = Actions::new()
        .add_action(Action::new(c2pa_action::CREATED).set_parameter("at", "07102022".to_owned())?);
...
fn read_source() -> Result<(), Error> {
     ...
     * but it shows the original image file did not have the generated claims
     * and is not altered during this process
     */
...
fn read_dest() -> Result<(), Error> {
    ...
    if let Some(manifest) = store.get_active() {
        ...
        match actions_result {
            ...
            Err(err) => {
                println!(
                    "had an issue getting the assertion with actions label: {:?}",
                    err
                );
            }
        }
...
fn main() -> Result<(), Error> {
    ...
    if write_result.is_err() {
        println!(
            "encountered an error writing the assertion: {:?}",
            write_result.err()
        );
    }
    ...
    if result.is_err() {
        println!(
            "had an issue reading a manifest from the first source: {:?}",
            result.err()
        );
    }
